...
func init() {
	handlers["dgb"] = &DgbHandler{}
}

type DgbHandler struct {
}

// GetAddress returns a wallet address.
func (hdlr *DgbHandler) GetAddress(ed *wallet.ExtendedData) (string, error) {
	...
	case wallet.XpubVersion:
		return wallet.MakeAddress(pk, 20, wallet.AddrP2PKH, wallet.AddrMain), nil
	case wallet.YpubVersion:
		return wallet.MakeAddress(pk, 20, wallet.AddrP2SH, wallet.AddrMain), nil
	}
