...
func init() {
	handlers["vtc"] = &VtcHandler{}
}

type VtcHandler struct {
}

// GetAddress returns a wallet address.
func (hdlr *VtcHandler) GetAddress(ed *wallet.ExtendedData) (string, error) {
	...
	case wallet.XpubVersion:
		return wallet.MakeAddress(pk, 28, wallet.AddrP2PKH, wallet.AddrMain), nil
	case wallet.YpubVersion:
		return wallet.MakeAddress(pk, 28, wallet.AddrP2SH, wallet.AddrMain), nil
	}
